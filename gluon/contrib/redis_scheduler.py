...
class RScheduler(Scheduler):
        ...
        if not mybackedstatus:
            r_server.hmset(
                ...
                dict(
                    ...
                    group_names=dumps(self.group_names),
                    is_ticker=False,
            ...
            if mybackedstatus == DISABLED:
                ...
                logger.info("........recording heartbeat (%s)", self.w_stats.status)
                r_server.hmset(
                    status_key,
                    dict(
                        last_heartbeat=self.dt2str(now),
            ...
            if counter % 5 == 0 or mybackedstatus == PICK:
                try:
                    logger.info("    freeing workers that have not sent heartbeat")
    ...
    def assign_tasks(self, db):
        ...
        for group in wkgroups.keys():
            ...
            for task in tasks:
                ...
                db((st.id == task.id) & (st.status.belongs((QUEUED, ASSIGNED)))).update(
                    **d
                )
            db.commit()
    ...
    def pop_task(self, db):
        ...
        for group in self.group_names:
            ...
            task_id = r_server.brpoplpush(
                queued_list, running_list, timeout=60 / len(self.group_names)
            )
            ...
            if task_id:
                ...
                task = db((st.id == task_id) & (st.status == QUEUED)).select().first()
                if not task:
    ...
    def report_task(self, task, task_report):
        ...
        is_expired = (
            task.stop_time and task.next_run_time > task.stop_time and True or False
        )
        status = (
            task.run_again
            and is_expired
    ...
    def queue_task(self, function, pargs=[], pvars={}, **kwargs):
        ...
        kwargs.update(
            function_name=function, task_name=tname, args=targs, vars=tvars, uuid=tuuid
        )
        if cronline:
            try:
