...
def render(
    text, extra={}, allowed={}, sep="p", image_mapper=lambda x: x, chapters=False
):
    #############################################################
...
def markmin2latex(data, image_mapper=lambda x: x, extra={}, wrapper=WRAPPER):
...
if __name__ == "__main__":
    parser = OptionParser()
    parser.add_option("-i", "--info", dest="info", help="markmin help")
    parser.add_option("-t", "--test", dest="test", action="store_true", default=False)
    parser.add_option(
        "-n", "--no_wrapper", dest="no_wrapper", action="store_true", default=False
    )
    parser.add_option(
        "-c",
        "--chapters",
        ...
        action="store_true",
        default=False,
        help="switch section for chapter",
    )
    parser.add_option(
        "-w",
        "--wrapper",
    ...
    if options.info:
        ...
        for f in args:
            ...
            try:
                ...
                fargs.close()
        content = "\n".join(content_data)
        output = markmin2latex(content, wrapper=wrapper, chapters=options.chapters)
        print(output)
