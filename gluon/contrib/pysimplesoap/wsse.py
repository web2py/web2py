...
class UsernameDigestToken(UsernameToken):
    ...
    def preprocess(self, client, request, method, args, kwargs, headers, soap_uri):
        ...
        wssenonce = usertoken.add_child(
            "wsse:Nonce", nonce.encode("base64")[:-1], ns=False
        )
        ...
        password = usertoken.add_child(
            "wsse:Password", digest.encode("base64")[:-1], ns=False
        )
...
class BinaryTokenSignature:
    ...
    def __init__(self, certificate="", private_key="", password=None, cacert=None):
        ...
        self.certificate = "".join(
            [line for line in open(certificate) if not line.startswith("---")]
        )
    ...
    def postprocess(self, client, response, method, args, kwargs, headers, soap_uri):
        ...
        self.__check(
            signed_info("Reference", ns=XMLDSIG_URI)("DigestMethod", ns=XMLDSIG_URI)[
                "Algorithm"
            ],
            XMLDSIG_URI + "sha1",
        )
        ...
        digest_value = str(
            signed_info("Reference", ns=XMLDSIG_URI)("DigestValue", ns=XMLDSIG_URI)
        )
