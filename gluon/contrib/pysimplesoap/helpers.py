    ...
    if not url_scheme in ("http", "https", "file"):
        for scheme in ("http", "https", "file"):
            try:
                ...
                return fetch(
                    tmp_url, http, cache, force_download, wsdl_basedir, headers
                )
        ...
        for e in children:
            ...
            if not t:
                ...
                if e["name"] and e.children():
                    ...
                    c = c.children()
                    process_element(
                        elements, t, c, et, xsd_uri, dialect, namespace, qualified
                    )
                else:
            ...
            if e["maxOccurs"] == "unbounded" or (
                uri == soapenc_uri and type_name == "Array"
            ):
            ...
            if (e["name"] is not None and not alias) or e["ref"]:
                ...
                log.debug(
                    "complexContent/simpleType/element %s = %s"
    ...
    if part_name:
        ...
        if len(parts) > 1:
            ...
            for part_name_key in parameter_order:
                ...
                if not part:
                    log.error(
                        "Part %s not found for %s" % (part_name_key, message_name)
                    )
                elif not new_msg:
                    new_msg = part.copy()
                else:
...
def preprocess_schema(
    ...
    for element in schema.children() or []:
        if element.get_local_name() in (
            ...
            log.debug(
                "Importing schema %s from %s" % (schema_namespace, schema_location)
            )
...
class Struct(dict):
    ...
    def __gt__(self, other):
        if isinstance(other, Struct):
            return (self.key[2], self.key[0], self.key[1]) > (
                other.key[2],
                other.key[0],
                other.key[1],
            )
