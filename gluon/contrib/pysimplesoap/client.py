...
class SoapFault(RuntimeError):
    ...
    def __repr__(self):
        return "SoapFault(faultcode = %s, faultstring %s, detail = %s)" % (
            repr(self.faultcode),
...
<%(soap_ns)s:Body><%(ns)s:%(method)s></%(ns)s:%(method)s></%(soap_ns)s:Body></%(soap_ns)s:Envelope>"""
    ...
    def call(self, method, *args, **kwargs):
        ...
        request = SimpleXMLElement(
            xml, namespace=self.__ns and self.namespace, prefix=self.__ns
        )
        ...
        if parameters and isinstance(parameters[0], SimpleXMLElement):
            body = request(
                "Body",
                ns=list(soap_namespaces.values()),
            )
            ...
            use_ns = (
                None
                if (self.__soap_server == "jetty" or self.qualified is False)
                else True
            )
        ...
        if self.__headers and not self.services:
            self.__call_headers = dict(
                [(k, v) for k, v in self.__headers.items() if not k.startswith("wsse:")]
            )
        # always extract WS Security header and send it (backward compatible)
        if "wsse:Security" in self.__headers and not self.plugins:
            warnings.warn(
                "Replace wsse:Security with UsernameToken plugin", DeprecationWarning
            )
        ...
        for plugin in self.plugins:
            plugin.preprocess(
                self, request, method, args, kwargs, self.__headers, soap_uri
            )
        ...
        if self.exceptions and response(
            "Fault", ns=list(soap_namespaces.values()), error=False
        ):
            detailXml = response(
                "detail", ns=list(soap_namespaces.values()), error=False
            )
            detail = None
            ...
            raise SoapFault(
                unicode(response.faultcode), unicode(response.faultstring), detail
            )

        # do post-processing using plugins (i.e. WSSE signature verification)
        for plugin in self.plugins:
            plugin.postprocess(
                self, response, method, args, kwargs, self.__headers, soap_uri
            )
    ...
    def get_operation(self, method):
        ...
        if not self.service_port:
            for service_name, service in self.services.items():
                for port_name, port in [port for port in service["ports"].items()]:
                    ...
                    raise RuntimeError(
                        "Cannot determine service in WSDL: "
        ...
        if not operation:
            raise RuntimeError(
                "Operation %s not found in WSDL: "
                "Service/Port Type: %s" % (method, self.service_port)
            )
        return operation
    ...
    def wsdl_call_with_args(self, method, args, kwargs):
        ...
        # parse results:
        resp = (
            response("Body", ns=soap_uri)
            .children()
            .unmarshall(output, strict=self.strict)
        )
        ...
        if input and args:
            ...
            for idx, arg in enumerate(args):
                ...
                if isinstance(arg, dict):
                    if key not in arg:
                        raise KeyError(
                            "Unhandled key %s. use client.help(method)" % key
                        )
                    d[key] = arg[key]
                else:
                    d[key] = arg
    ...
    def wsdl_validate_params(self, struct, value):
        ...
        # traverse tree
                ...
                for key in value:
                    if key not in struct:
                        ...
                        (
                            ...
                            next_errors,
                            next_warnings,
                        ) = self.wsdl_validate_params(struct[key], value[key])
                        if not next_valid:
                            valid = False
                        errors.extend(next_errors)
                        warnings.extend(next_warnings)
            ...
            valid = False
            errors.append(
                "Type mismatch. parameter(%s): %s, value(%s): %s"
                % (type(struct), struct, type(value), value)
            )
    ...
    def _url_to_xml_tree(self, url, cache, force_download):
        ...
        xml = fetch(
            url, self.http, cache, force_download, self.wsdl_basedir, self.http_headers
        )
        # Parse WSDL XML:
    ...
    def _xml_tree_to_services(self, wsdl, cache, force_download):
            ...
            for part in message("part", error=False) or []:
                ...
                if type_uri == self.xsd_uri:
                    ...
                    if not fn:
                        ...
                        fn = elements.get(
                            make_key(element_name, "complexType", type_uri)
                        )
                        od = Struct()
                        od[part_name] = fn
                        od.namespaces[None] = type_uri
        ...
        for port_type_node in wsdl.portType:
            ...
            for operation_node in port_type_node.operation:
                ...
                op["documentation"] = (
                    unicode(operation_node("documentation", error=False)) or ""
                )
        ...
        for binding_node in wsdl.binding:
            ...
            soap_binding = binding_node(
                "binding", ns=list(soap_uris.values()), error=False
            )
            ...
            for operation_node in binding_node.operation:
                op_name = operation_node["name"]
                op_op = operation_node(
                    "operation", ns=list(soap_uris.values()), error=False
                )
                ...
                parts_input_headers = []
                headers = input and input(
                    "header", ns=list(soap_uris.values()), error=False
                )
                ...
                if "input_msg" in op:
                    ...
                    for input_header in parts_input_headers:
                        ...
                        hdr = get_message(
                            messages, header_msg or op["input_msg"], header_part
                        )
                        if hdr:
                            headers.update(hdr)
                        else:
                ...
                body = output and output(
                    "body", ns=list(soap_uris.values()), error=False
                )
                parts_output_body = body and body["parts"] or None
                if "output_msg" in op:
                    op["output"] = get_message(
                        messages, op["output_msg"], parts_output_body
                    )
                    # Remove temporary property
                    del op["output_msg"]
                else:
        ...
        elements = list(e for e in elements.values() if type(e) is type) + sorted(
            e for e in elements.values() if not (type(e) is type)
        )
        e = None
        self.elements = []
    ...
    def wsdl_parse(self, url, cache=False):
        ...
        if cache:
            ...
            if os.path.exists(filename_pkl):
                ...
                # sanity check:
                if (
                    pkl["version"][:-1] != __version__.split(" ")[0][:-1]
                    or pkl["url"] != url
                ):
                    warnings.warn(
        ...
        for service in self.services:
            ...
            for port in ports:
                ...
                for operation in sorted(operations):
                    ...
                    if output:
                        output = list(operation["output"].values())[0]
                    s += "\n     %s(%s)" % (operation["name"], input_str[:-2])
                    s += "\n      > %s" % output
