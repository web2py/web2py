    ...
    def __init__(self, node, write, **kw):
        ...
        if node.nodeType == Node.DOCUMENT_NODE:
            ...
            if not _inclusive(self):
                inherited, unused = _inclusiveNamespacePrefixes(
                    node, self._inherit_context(node), self.unsuppressedPrefixes
                )
    ...
    def _do_element(self, node, initial_other_attrs=[], unused=None):
        ...
        for a in initial_other_attrs + _attrs(node):
            if a.namespaceURI == XMLNS.BASE:
                ...
                if inclusive or (
                    in_subset and _in_subset(self.subset, a)
        ...
        if in_subset:
            ...
            for n, v in ns_local.items():
                ...
                if (n, v) not in ns_rendered.items():
                    if inclusive or _utilized(
                        n, node, other_attrs, self.unsuppressedPrefixes
                    ):
