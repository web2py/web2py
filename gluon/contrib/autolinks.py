...
def googledoc_viewer(url):
    return (
        '<iframe src="https://docs.google.com/viewer?url=%s&embedded=true" style="max-width:100%%"></iframe>'
        % urllib_quote(url)
    )
...
def expand_html(html, cdict=None):
    ...
    for txt in soup.findAll(text=True):
        if not txt.parent.name in (
            ...
            ntxt = regex_link.sub(lambda match: expand_one(match.group(0), cdict), txt)
