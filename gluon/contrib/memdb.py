...
def assert_filter_fields(*fields):
    for field in fields:
        if isinstance(field, (Field, Expression)) and field.type in ["text", "blob"]:
...
class Table(DALStorage):
        ...
        for k in self.fields:
            ...
            if field.type[:9] == "reference":
                ...
                self._db[referenced]._referenced_by.append(
                    (self._tablename, field.name)
                )
            elif (
                not field.type in self._db._translator
    ...
    def update(self, id, **fields):
        for field in fields:
            if not field in fields and self[field].default is not None:
...
class Set(object):
    ...
    def __call__(self, where):
        if isinstance(self.where, QueryException) or isinstance(where, QueryException):
            raise SyntaxError(
