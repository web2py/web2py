...
class DropboxAccount(object):
    ...
    def __init__(
        ...
        self.sess = session.DropboxSession(self.key, self.secret, self.access_type)
    ...
    def login_form(self):
        ...
        dropbox_url = self.sess.build_authorize_url(request_token, self.login_url)
...
def use_dropbox(auth, filename="private/dropbox.key", **kwargs):
    ...
    if os.path.exists(path):
        ...
        login_url = "http://%s/%s/default/user/login" % (host, request.application)
