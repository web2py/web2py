...
def ldap_auth(
    ...
    def ldap_auth_aux(
        ...
        if password == "":  # http://tools.ietf.org/html/rfc4513#section-5.1.2
            ...
            return False
        logger.debug(
            "mode: [%s] manage_user: [%s] custom_scope: [%s]"
            " manage_groups: [%s]"
        ...
        try:
            ...
            if ldap_mode == "company":
                ...
                company_search_result = con.search_s(
                    ldap_basedn, ldap.SCOPE_SUBTREE, filter, attrs
                )
    ...
    def is_user_in_allowed_groups(
        username, password=None, allowed_groups=allowed_groups
    ):
        """
    ...
    def do_manage_groups(username, password=None, group_mapping={}, db=db):
        ...
        try:
            ...
            if db_can_join:
                ...
                group_ids = [
                    x.group_id
                    for x in db_group_search.select(
            ...
            if db_group_search.count() > 0:
                for group in db_group_search.select(
                    db.auth_group.id, db.auth_group.role, distinct=True
                ):
                    db_group_id[group.role] = group.id
                    db_groups_of_the_user.append(group.role)
            logger.debug(
                "db groups of user %s: %s" % (username, str(db_groups_of_the_user))
            )
            ...
            for group_to_del in db_groups_of_the_user:
                if ldap_groups_of_the_user.count(group_to_del) == 0:
                    db(
                        (db.auth_membership.user_id == db_user_id)
                        & (db.auth_membership.group_id == db_group_id[group_to_del])
                    ).delete()
                ...
                if db_groups_of_the_user.count(group_to_add) == 0:
                    if db(db.auth_group.role == group_to_add).count() == 0:
                        gid = db.auth_group.insert(
                            role=group_to_add, description="Generated from LDAP"
                        )
                    else:
                        gid = (
                            db(db.auth_group.role == group_to_add)
    ...
    def init_ldap(
        ...
        if secure:
            ...
            if not ldap_port:
                ldap_port = 389
            con = ldap.initialize("ldap://" + ldap_server + ":" + str(ldap_port))
        if tls:
            con.start_tls_s()
        return con

    def get_user_groups_from_ldap(
        username,
        password=None,
        base_dn=base_dn,
        ldap_binddn=bind_dn,
