...
class JobGraph(object):
    ...
    def add_deps(self, task_parent, task_child):
        ...
        self.db.scheduler_task_deps.insert(
            task_parent=task_parent, task_child=task_child, job_name=self.job_name
        )
    ...
    def validate(self, job_name=None):
        ...
        try:
            ...
            extra_items_in_deps = reduce(set.union, nested_dict.values()) - set(
                nested_dict.keys()
            )
            ...
            while True:
                ...
                nested_dict = dict(
                    (item, (dep - ordered))
                    for item, dep in nested_dict.items()
                    if item not in ordered
                )
            assert not nested_dict, (
                "A cyclic dependency exists amongst %r" % nested_dict
            )
            db.commit()
            return rtn
        except Exception:
...
class CronParser(object):
                ...
                for val in vals:
                    ...
                    if i in ("dow", "mon") and "-" in val and "/" not in val:
                        ...
                        if isnum:
                            ...
                            val = "-".join(
                                [str(refdict.get(v, "")) for v in val.split("-")]
                            )
    ...
    def _get_next_dom(sched, task):
        if task["dom"] == ["l"]:
            ...
            try:
                ...
                last_feb = 28
            lastdayofmonth = [31, last_feb, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31]
    ...
    def next(self):
        ...
        while x < 1000:  # avoid potential max recursions
            ...
            raise ValueError(
                "Potential bug found, please submit your "
...
def executor(retq, task, outq):
    ...
    W2P_TASK = Storage({"id": task.task_id, "uuid": task.uuid, "run_id": task.run_id})
    ...
    try:
        if task.app:
            ...
            _env = env(
                a=a, c=c, import_models=True, extra_request={"is_scheduler": True}
            )
...
class Scheduler(threading.Thread):
    ...
    def execute(self, task):
        ...
        if self.use_spawn:
            ...
            self.process = p = multiprocessing.Process(
                target=executor, args=(retq, task, outq)
            )
    ...
    def loop(self, worker_name=None):
        ...
        try:
            ...
            while self.have_heartbeat:
                ...
                if is_disabled:
                    logger.debug(
                        "Someone stopped me, sleeping until better" " times come (%s)",
                        self.w_stats.sleep,
                    )
                    self.sleep()
                    continue
    ...
    def pop_task(self):
        ...
        grabbed = db(
            (st.assigned_worker_name == self.worker_name) & (st.status == ASSIGNED)
        )
        ...
        if task.cronline:
            cron_recur = CronParser(task.cronline, now.replace(second=0, microsecond=0))
    ...
    def report_task(self, task, task_report):
        ...
        is_expired = task.stop_time and task.next_run_time > task.stop_time or False
        status = (
            task.run_again
            and is_expired
            and EXPIRED
    ...
    def adj_hibernation(self):
        ...
        with self.w_stats_lock:
            if self.w_stats.status == DISABLED:
                ...
                hibernation = (
                    wk_st + HEARTBEAT if wk_st < MAXHIBERNATION else MAXHIBERNATION
                )
                ...
                if not row:
                    ...
                    if backed_status == DISABLED:
                        ...
                        db(sw.worker_name == self.worker_name).update(
                            last_heartbeat=now, worker_stats=self.w_stats
                        )
                        ...
                        if backed_status == STOP_TASK:
                            ...
                            self.terminate_process()
                        logger.debug(
                            "........recording heartbeat (%s)", self.w_stats.status
                        )
                        db(sw.worker_name == self.worker_name).update(
                            last_heartbeat=now, status=ACTIVE, worker_stats=self.w_stats
                        )
    ...
    def being_a_ticker(self):
        ...
        all_active = db((sw.worker_name != my_name) & (sw.status == ACTIVE)).select(
            sw.is_ticker, sw.worker_name
        )
        ...
        if not ticker:
            ...
            if not_busy:
                ...
                # NOTE: this old way could easily cause deadlocks
                # db(sw.worker_name == my_name).update(is_ticker=True)
                # db(sw.worker_name != my_name).update(is_ticker=False)
                #
                # So we use an update expression
                db(sw.id > 0).update(is_ticker=sw.worker_name == my_name)
    ...
    def assign_tasks(self):
        ...
        db((st.status.belongs((QUEUED, ASSIGNED))) & (st.stop_time < now)).update(
            status=EXPIRED
        )
                ...
                if ws:
                    if task.broadcast:
                        ...
                        if task.period:
                            ...
                            # must be cronline
                            cron_recur = CronParser(
                                task.cronline, now.replace(second=0, microsecond=0)
                            )
                            next_run_time = cron_recur.next()
                        db(st.id == task.id).update(
                            times_run=task.times_run + 1,
                            next_run_time=next_run_time,
                            last_run_time=now,
                        )
                        ...
                        d = dict(status=ASSIGNED, assigned_worker_name=assigned_wn)
                        db(
                            (st.id == task.id) & (st.status.belongs((QUEUED, ASSIGNED)))
                        ).update(**d)
    ...
    def set_worker_status(
        ...
        if not limit:
            for group in group_names:
                db(
                    (ws.group_names.contains(group)) & (~ws.status.belongs(exclusion))
    ...
    def terminate(self, group_names=None, limit=None, worker_name=None):
        ...
        """
        self.set_worker_status(
            group_names=group_names, action=TERMINATE, exclude=[KILL], limit=limit
        )
    ...
    def kill(self, group_names=None, limit=None, worker_name=None):
        ...
        processing a task."""
        self.set_worker_status(group_names=group_names, action=KILL, limit=limit)
    ...
    def task_status(self, ref, output=False):
        ...
        row = (
            db(q)
            .select(*fields, **dict(orderby=orderby, left=left, limitby=(0, 1)))
            .first()
        )
        if row and output:
            row.result = (
    ...
    def stop_task(self, ref):
        ...
        if isinstance(ref, integer_types):
            ...
            raise SyntaxError("You can retrieve results only by id or uuid")
        ...
        if task.status == "RUNNING":
            ...
            rtn = db(q).update(stop_time=self.now(), enabled=False, status=STOPPED)
        return rtn
