...
def run(
    ...
    if not os.path.exists(adir):
        if (
            not cron_job
            and not scheduler_job
            and sys.stdin
            and not sys.stdin.name == "/dev/null"
        ):
    ...
    _env = env(
        a, c=c, f=f, import_models=import_models, extra_request=extra_request, dir=adir
    )

    if c:
        pyfile = os.path.join("applications", a, "controllers", c + ".py")
        pycfile = os.path.join(
            "applications", a, "compiled", "controllers.%s.%s.pyc" % (c, f)
        )
        if (cron_job and os.path.isfile(pycfile)) or not os.path.isfile(pyfile):
