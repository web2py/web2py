...
if sys.version_info < (2, 7) or (3, 0) < sys.version_info < (3, 5):
    ...
    sys.stderr.write(
        "Warning: web2py requires at least Python 2.7/3.5"
...
class web2pyDialog(object):
    ...
    def __init__(self, root, options):
        ...
        servermenu.add_command(label="View httpserver.log", command=item)
        ...
        helpmenu.add_command(label="Home Page", command=item)
        ...
        helpmenu.add_command(label="About", command=item)
        ...
        if options.taskbar:
            self.root.protocol("WM_DELETE_WINDOW", lambda: self.quit(True))
        else:
        ...
        self.logoarea = tkinter.Canvas(
            self.root, background=bg_color, width=300, height=300
        )
        ...
        self.bannerarea = tkinter.Canvas(self.root, bg=bg_color, width=300, height=300)
        ...
        tkinter.Label(
            self.root, text="Server IP:", bg=bg_color, justify=tkinter.RIGHT
        ...
        # Port
        tkinter.Label(
            self.root, text="Server Port:", bg=bg_color, justify=tkinter.RIGHT
        ).grid(row=shift, column=1, pady=10, sticky=sticky)
        ...
        # Password
        tkinter.Label(
            self.root, text="Choose Password:", bg=bg_color, justify=tkinter.RIGHT
        ).grid(row=shift + 1, column=1, sticky=sticky)
        ...
        # Prepare the canvas
        self.canvas = tkinter.Canvas(self.root, width=400, height=100, bg="black")
        self.canvas.grid(row=shift + 2, column=1, columnspan=2, pady=5, sticky=sticky)
        self.canvas.after(1000, self.update_canvas)

        # Prepare the frame
        frame = tkinter.Frame(self.root)
        frame.grid(row=shift + 3, column=1, columnspan=2, pady=5, sticky=sticky)

        # Start button
        self.button_start = tkinter.Button(
            frame, text="start server", command=self.start
        )
    ...
    def update_schedulers(self, start=False):
        ...
        available_apps = [
            arq
            for arq in os.listdir(applications_folder)
        ...
        with self.scheduler_processes_lock:
            ...
            for arq in available_apps:
                if arq not in self.scheduler_processes:
                    ...
                    self.schedmenu.add_command(label="start %s" % arq, command=item)
                if arq in self.scheduler_processes:
                    item = lambda a=arq: self.try_stop_scheduler(a)
                    self.schedmenu.add_command(label="stop %s" % arq, command=item)
    ...
    def start_schedulers(self, app):
        ...
        with self.scheduler_processes_lock:
            ...
            print(
                "Currently running %s scheduler processes"
    ...
    def connect_pages(self):
        ...
        for arq in available_apps:
            ...
            self.pagesmenu.add_command(label=url, command=item)
