...
class TestTranslations(unittest.TestCase):
    def setUp(self):
        if os.path.isdir("gluon"):
            ...
            self.langpath = os.path.realpath("../../applications/welcome/languages")
    ...
    def test_plain(self):
        ...
        self.assertEqual(str(T.M("**Hello World**")), "<strong>Hello World</strong>")
        # sub_tuple testing
        ...
        self.assertEqual(str(T("%(key)i %%{?not_one(key)}", dict(key=1))), "1 ")
        self.assertEqual(str(T("%(key)s %%{?not_one(key)}", dict(key=2))), "2 not_one")
        self.assertEqual(str(T("%(key)i %%{?not_one(key)}", dict(key=0))), "0 not_one")
        self.assertEqual(str(T("%(key)s %%{?one?not_one(key)}", dict(key=1))), "1 one")
        self.assertEqual(
            str(T("%(key)i %%{?one?not_one(key)}", dict(key=2))), "2 not_one"
        )
        self.assertEqual(
            str(T("%(key)s %%{?one?not_one(key)}", dict(key=0))), "0 not_one"
        )
        self.assertEqual(str(T("%(key)i %%{?one?(key)}", dict(key=1))), "1 one")
        self.assertEqual(str(T("%(key)s %%{?one?(key)}", dict(key=2))), "2 ")
        self.assertEqual(str(T("%(key)i %%{?one?(key)}", dict(key=0))), "0 ")
        self.assertEqual(str(T("%(key)s %%{??not_one(key)}", dict(key=1))), "1 ")
        self.assertEqual(str(T("%(key)i %%{??not_one(key)}", dict(key=2))), "2 not_one")
        self.assertEqual(str(T("%(key)s %%{?not_one(key)}", dict(key=1))), "1 ")
        self.assertEqual(str(T("%(key)i %%{?not_one(key)}", dict(key=0))), "0 not_one")
        self.assertEqual(
        ...
        self.assertEqual(
            str(T("%(key)s %%{?one?two_or_more?(key)}", dict(key=2))), "2 two_or_more"
        )
        self.assertEqual(
        ...
        self.assertEqual(
            str(T("%(key)i %%{??two_or_more?zero(key)}", dict(key=2))), "2 two_or_more"
        )
        self.assertEqual(
            str(T("%(key)s %%{??two_or_more?zero(key)}", dict(key=0))), "0 zero"
        )
        self.assertEqual(str(T("%(key)i %%{??two_or_more?(key)}", dict(key=1))), "1 ")
        self.assertEqual(str(T("%(key)s %%{??two_or_more?(key)}", dict(key=0))), "0 ")
        self.assertEqual(
...
class TestDummyApp(unittest.TestCase):
    def setUp(self):
        pjoin = os.path.join
        self.apppath = os.path.abspath(
            pjoin(os.path.dirname(os.path.abspath(__file__)), "dummy")
        )
...
"""
        ...
        with open(
            pjoin(self.apppath, "controllers", "default.py"), "w"
        ) as testcontroller:
...
"""
        ...
        with open(
            pjoin(self.apppath, "views", "default", "index.html"), "w"
        ) as testview:
            testview.write(
                """
<html>
