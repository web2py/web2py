...
REGEX_URL = re.compile(
    r"^/((?P<a>\w+)(/(?P<c>\w+)(/(?P<z>(?P<f>\w+)(\.(?P<e>[\w.]+))?(?P<s>.*)))?)?)?$"
)
...
ROUTER_BASE_KEYS = set(
    ("applications", "default_application", "domains", "path_prefix")
)
...
def url_out(
    ...
    if not scheme or scheme is True:
        scheme = (
            request.env.get("wsgi_url_scheme", "http").lower() if request else "http"
        )
    if host:
...
def try_rewrite_on_error(http_response, request, environ, ticket=None):
    ...
    if status >= 399 and THREAD_LOCAL.routes.routes_onerror:
        keys = set(
            (
                "%s/%s" % (request.application, status),
...
def load(routes="routes.py", app=None, data=None, rdict=None):
    ...
    if app is None:
        ...
        for appname in os.listdir(apppath):
            if (
                not appname.startswith(".")
                and isdir(abspath(apppath, appname))
                and isdir(abspath(apppath, appname, "controllers"))
            ):
                    ...
                    if appname in routers:
                        for key in routers[appname].keys():
                            if key in ROUTER_BASE_KEYS:
                                raise SyntaxError(
                                    "BASE-only key '%s' in router '%s'" % (key, appname)
                                )
...
def compile_regex(k, v, env=None):
    ...
    if k.find("://") < 0:
        ...
        if i < 0:
            raise SyntaxError(
                "routes pattern syntax error: path needs leading '/' [%s]" % k0
            )
...
def load_routers(all_apps):
    ...
    for app in routers:
        ...
        if app not in all_apps:
            ...
            if app != "BASE":
                ...
                if keys:
                    raise SyntaxError(
                        "BASE-only key(s) %s in router '%s'" % (tuple(keys), app)
                    )
    ...
    if (
        isinstance(routers.BASE.applications, str)
        and routers.BASE.applications == "ALL"
    ):
...
def regex_uri(e, regexes, tag, default=None):
    ...
    key = "%s:%s://%s:%s %s" % (
        ...
        e.get("wsgi.url_scheme", "http").lower(),
        host,
...
def invalid_url(routes):
    raise HTTP(
        400, routes.error_message % "invalid request", web2py_error="invalid path"
    )
...
def regex_filter_out(url, e=None):
    ...
    if routes.routes_out:
        ...
        if e:
            ...
            items[0] = "%s:%s://%s:%s %s" % (
                e.get("remote_addr", ""),
                e.get("wsgi_url_scheme", "http").lower(),
                host,
                e.get("request_method", "get").lower(),
                items[0],
            )
...
def filter_url(
    ...
    """
    match = re.match(
        r"^(?P<scheme>http|https|HTTP|HTTPS)\://(?P<host>[^/]*)(?P<uri>.*)", url
    )
...
class MapUrlIn(object):
    ...
    def __init__(self, request=None, env=None):
        ...
        if not self.host:
            (self.host, self.port) = (
                self.env.get("SERVER_NAME"),
                self.env.get("SERVER_PORT"),
            )
        if not self.host:
            self.host = "localhost"
            self.port = "80"
    ...
    def map_app(self):
        ...
        arg0 = self.harg0
        if (
            not base.exclusive_domain
            and base.applications
            and arg0 in base.applications
        ):
            ...
            ) = base.domains[(self.host, self.port)]
        ...
        if self.application not in routers and (
            self.application != THREAD_LOCAL.routes.default_application
            or self.application == "welcome"
        ):
            raise HTTP(
                400,
                THREAD_LOCAL.routes.error_message % "invalid request",
        ...
        self.default_controller = (
            self.domain_controller or self.router.default_controller
        )
...
class MapUrlOut(object):
    ...
    def omit_acf(self):
        ...
        #
        default_controller = (
            ((self.application == self.domain_application) and self.domain_controller)
            or router.default_controller
            or ""
        )
        ...
        if (
            ...
            and self.function == self.default_function
        ):
            self.omit_function = True
    ...
    def build_acf(self):
        ...
        if self.map_hyphen:
            ...
            if self.controller != "static" and not self.controller.startswith(
                "static/"
            ):
...
def map_url_in(request, env, app=False):
    ...
    if (
        map.languages
        and map.map_static is False
        and map.arg0 == "static"
        and map.args(1) in map.languages
    ):
        map.map_controller()
        map.map_language()
    else:
