            ...
            try:
                with self.run_lock:  # waiting for run_lock.release()
                    ...
                    if proc.returncode != 0:
                        logger.warning(
                            "Worker %s: %r call returned code %s:\n%s\n%s",
                            self.name,
                            cmd,
                            proc.returncode,
                            stdoutdata,
                        ...
                        logger.debug(
                            "Worker %s: %r call returned success:\n%s",
                            self.name,
                            cmd,
                            stdoutdata,
                        )
...
def crondance(applications_parent, ctype="hard", startup=False, apps=None):
    ...
    try:
        ...
        for app in apps:
            ...
            try:
                cronlines = [
                    line.strip() for line in fileutils.readlines_file(crontab, "rt")
                ]
                lines = [
            ...
            for task in tasks:
                ...
                logger.info(
                    "%s cron: %s executing %r in %s at %s",
                    ctype,
                    app,
                    task.get("cmd"),
                    os.getcwd(),
                ...
                try:
                    if not _launcher(commands):
                        logger.warning(
                            "no thread available, cannot execute %r", task["cmd"]
                        )
                except Exception:
