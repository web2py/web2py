...
def parse_legacy(version="Version 1.99.0 (2011-09-19 08:23:26)"):
    ...
    """
    re_version = re.compile(
        r"[^\d]+ (\d+)\.(\d+)\.(\d+)\s*\((?P<datetime>.+?)\)\s*(?P<type>[a-z]+)?"
    )
...
def check_credentials(
    ...
    if request.env.web2py_runtime_gae:
        ...
        if users.is_current_user_admin():
            ...
            login_html = (
                '<a href="%s">Sign in with your google account</a>.'
...
def create_missing_folders():
    ...
    """
    for p in (global_settings.gluon_parent, abspath("site-packages", gluon=True), ""):
        add_path_first(p)
...
def add_path_first(path):
    sys.path = [path] + [
        p for p in sys.path if (not p == path and not p == (path + "/"))
    ]
    if not global_settings.web2py_runtime_gae:
