        ...
        #######################################################
            ...
            if sign:
                ...
                try:
                    ...
                    self.error = (
                        "Something went wrong on certificate / private key loading: <%s>"
                        % str(e)
                    )
                    return False
                try:
...
def addrow(form, a, b, c, style, _id, position=-1):
    if style == "divs":
        ...
        form[0].insert(
            position, TR(TD(LABEL(a), _class="w2p_fl"), TD(c, _class="w2p_fc"))
        )
        ...
        form[0].insert(
            position,
            DIV(
                LABEL(a, _class="control-label col-sm-3"),
                DIV(b, SPAN(c, _class="help-block"), _class="col-sm-9"),
...
class AuthJWT(object):
    ...
    def refresh_token(self, orig_payload):
        ...
        expires = now + self.expiration
        orig_payload.update(
            orig_iat=orig_iat, iat=now, exp=expires, hmac_key=web2py_uuid()
        )
    ...
    def get_jwt_token_from_request(self, token_param="_token"):
        ...
        if token_in_header:
            ...
            if token is None:
                raise HTTP(
                    400,
                    "JWT header not found and JWT parameter {} missing in request".format(
                        token_param
                    ),
                )
...
class Auth(AuthAPI):
    ...
    def __init__(
        ...
        url_login = URL(
            controller, function, args="login", extension=propagate_extension
        )
        ...
        messages.update(
            ajax_failed_authentication=DIV(
                H4("NOT AUTHORIZED"),
                "Please ",
                A(
                    "login",
    ...
    def navbar(
        ...
        if URL() == action:
            ...
            next = "?_next=" + urllib_quote(
                URL(args=request.args, vars=request.get_vars)
            )
        href = lambda function: "%s/%s%s" % (
            action,
            function,
        ...
        if self.user_id:  # User is logged in
            ...
            if "profile" not in self.settings.actions_disabled:
                items.append(
                    {"name": T("Profile"), "href": href("profile"), "icon": "icon-user"}
                )
            ...
            if callable(user_identifier):
                ...
                or type(user_identifier).__name__ == "lazyT"
            ) and re.search(r"%\(.+\)s", user_identifier):
                user_identifier = user_identifier % self.user
            if not user_identifier:
                user_identifier = ""
        else:  # User is not logged in
            items.append(
                {"name": T("Log In"), "href": href("login"), "icon": "icon-off"}
            )
            ...
            if "request_reset_password" not in self.settings.actions_disabled:
                items.append(
                    {
                        "name": T("Lost password?"),
                        "href": href("request_reset_password"),
                        "icon": "icon-lock",
                    }
                )
            if (
                self.settings.use_username
                and "retrieve_username" not in self.settings.actions_disabled
            ):
                items.append(
                    {
                        "name": T("Forgot username?"),
        ...
        def bootstrap3():  # Default web2py scaffolding
            ...
            self.bar = UL(
                LI(
                    Anr(
                        I(_class=rename("icon " + items[0]["icon"])),
                        " " + items[0]["name"],
                        _href=items[0]["href"],
                    )
                ),
            ...
            for item in items:
                self.bar.insert(
                    ...
                    LI(
                        Anr(
                            I(_class=rename("icon " + item["icon"])),
                            " " + item["name"],
                            _href=item["href"],
                        )
                    ),
        ...
        if mode in options and callable(options[mode]):
            ...
            if self.user_id:
                self.bar = SPAN(
                    ...
                    user_identifier,
                    s1,
                    Anr(items[0]["name"], _href=items[0]["href"]),
                    s3,
                    _class="auth_navbar",
                )
            else:
    ...
    def define_tables(
        ...
        signature_list = (
            super(Auth, self)
            .define_tables(username, signature, migrate, fake_migrate)
        ...
        if settings.enable_tokens:
            extra_fields = (
                settings.extra_fields.get(settings.table_token_name, [])
                + signature_list
            )
            if settings.table_token_name not in db.tables:
                db.define_table(
                    ...
                    Field(
                        "expires_on",
                        "datetime",
                        default=datetime.datetime(2999, 12, 31),
                    ),
        ...
        if settings.cas_provider:  # THIS IS NOT LAZY
            ...
            if not maps:
                ...
                maps = dict(
                    (name, lambda v, n=name: v.get(n, None))
                    for name in table_user.fields
                    if name != "id" and table_user[name].readable
                )
                maps["registration_id"] = lambda v, p=settings.cas_provider: "%s/%s" % (
                    p,
                    v["user"],
                )
            actions = [
                settings.cas_actions["login"],
                settings.cas_actions["servicevalidate"],
                settings.cas_actions["logout"],
            ]
            settings.login_form = CasAuth(
                casversion=2, urlbase=settings.cas_provider, actions=actions, maps=maps
            )
        return self

    def get_or_create_user(self, keys, update_fields=["email"], login=True, get=True):
        """
        ...
        # make new user
        if (
            "registration_id" in checks
            and user
            and user.registration_id
            and (
                "registration_id" not in keys
                or user.registration_id != str(keys["registration_id"])
            )
        ):
    ...
    def _get_login_settings(self):
        table_user = self.table_user()
        userfield = self.settings.login_userfield or (
            "username"
            if self.settings.login_userfield or "username" in table_user.fields
            else "email"
        )
        passfield = self.settings.password_field
        return Storage(
            {"table_user": table_user, "userfield": userfield, "passfield": passfield}
        )
    ...
    def login_bare(self, username, password):
        ...
        if user and user.get(settings.passfield, False):
            ...
            if (
                user.registration_key is None or not user.registration_key.strip()
    ...
    def register_bare(self, **fields):
        ...
        if fields.get(settings.passfield):
            fields[settings.passfield] = settings.table_user[
                settings.passfield
            ].validate(fields[settings.passfield], None)[0]
        if not fields.get(settings.userfield):
            raise ValueError("register_bare: userfield not provided or invalid")
        user = self.get_or_create_user(
            fields, login=False, get=False, update_fields=self.settings.update_fields
        )
        if not user:
            ...
            if version == 1:
                ...
                message = build_response(
                    TAG["cas:authenticationFailure"](
                        "Ticket %s not recognized" % ticket, _code="INVALID TICKET"
                    )
                )
    ...
    def login(
        ...
        if next is DEFAULT:
            ...
            if user_next:
                ...
                if external[0].lower() in ["http", "https", "ftp"]:
                    ...
                    self.when_is_logged_in_bypass_next_in_url(
        ...
        table_user = self.table_user()
        if "username" in table_user.fields or not settings.login_email_validate:
        ...
        if session.auth_two_factor_user is None:
            ...
            if settings.login_form == self:
                ...
                if form.accepts(
                    request,
                    session if self.csrf_prevention else None,
                    formname="login",
                    dbio=False,
                    ...
                    if user:
                        ...
                        if (temp_user.registration_key or "").startswith("pending"):
                            ...
                            temp_user.registration_key is not None
                        ...
                        for login_method in settings.login_methods:
                            if login_method != self and login_method(
                                request.vars[username], request.vars[passfield]
                            ):
                    ...
                    if not user:
                        self.log_event(
                            self.messages["login_failed_log"], request.post_vars
                        )
                        # invalid login
                        session.flash = (
                            specific_error
                            if self.settings.login_specify_error
                            else self.messages.invalid_login
                        )
                ...
                if cas_user:
                    ...
                    user = self.get_or_create_user(
                        table_user._filter_fields(cas_user), settings.update_fields
                    )
                    ...
                    redirect(cas.login_url(next), client_side=settings.client_side)
        ...
        if user and (
            ...
            session.auth_two_factor_enabled = self.has_membership(
            ...
            if session.auth_two_factor_user is None and user is not None:
                ...
                if not two_factor_methods:
                    ...
                    self.settings.mailer.send(
                        ...
                        message=self.messages.retrieve_two_factor_code.format(
            ...
            if form.accepts(
                ...
                if self.settings.two_factor_onvalidation:
                    for (
                        ...
                        try:
                            session.auth_two_factor = two_factor_onvalidation(
                                session.auth_two_factor_user,
                ...
                if form.vars["authentication_code"] == str(session.auth_two_factor):
                    ...
                    if (
                        session.auth_two_factor_enabled
                        and session.auth_two_factor_tries_left < 1
                    ):
                        # Exceeded maximum allowed tries for this code. Require user to enter
                        # username and password again.
                        user = None
                        ...
                        # back to the main login form
                        redirect(next, client_side=settings.client_side)
                    response.flash = self.messages.invalid_two_factor_code.format(
                        session.auth_two_factor_tries_left
                    )
                    return form
            else:
        ...
        if user:
            ...
            session.auth.expiration = (
                request.post_vars.remember_me
                and settings.long_expiration
                or settings.expiration
            )
    ...
    def register(
        ...
        if not table_user[username].requires:
            ...
            if not any(
                [
                    isinstance(validator, IS_NOT_IN_DB)
                    for validator in table_user[username].requires
                ]
            ):
        ...
        if self.settings.register_verify_password:
            if self.settings.register_fields is None:
                self.settings.register_fields = [
                    f.name for f in table_user if f.writable and not f.compute
                ]
        ...
        if form.accepts(
            ...
            if (
                self.settings.registration_requires_approval
                and not self.settings.registration_requires_verification
            ):
                table_user[form.vars.id] = dict(registration_key="pending")
                session.flash = self.messages.registration_pending
            elif (
                not self.settings.registration_requires_verification
                or self.settings.login_after_registration
            ):
    ...
    def retrieve_username(
        ...
        captcha = self.settings.retrieve_username_captcha or (
            self.settings.retrieve_username_captcha is not False
            and self.settings.captcha
        )
        ...
        table_user.email.requires = [
            IS_IN_DB(
                self.db, table_user.email, error_message=self.messages.invalid_email
            )
        ]
        form = SQLFORM(
            table_user,
    ...
    def bulk_register(self, max_emails=100):
        ...
        form = SQLFORM.factory(
            Field(
                "subject",
                "string",
                default=self.messages.bulk_invite_subject,
            ...
            Field(
                "message",
                "text",
                default=self.messages.bulk_invite_body,
                requires=IS_NOT_EMPTY(),
            ),
            formstyle=self.settings.formstyle,
        )
        ...
        if form.process().accepted:
            ...
            for email in emails[:max_emails]:
                if self.table_user()(email=email):
                    ...
                    if self.email_registration(
                        form.vars.subject, form.vars.message, user
                    ):
            ...
            form = DIV(
                H4("Emails sent"),
                UL(*[A(x, _href="mailto:" + x) for x in emails_sent]),
                H4("Emails failed"),
                UL(*[A(x, _href="mailto:" + x) for x in emails_fail]),
                H4("Emails existing"),
                UL(*[A(x, _href="mailto:" + x) for x in emails_exist]),
            )
        return form
    ...
    def request_reset_password(
        ...
        captcha = self.settings.retrieve_password_captcha or (
            self.settings.retrieve_password_captcha is not False
            and self.settings.captcha
        )
        ...
        if form.accepts(
            ...
            if not user:
                ...
                redirect(
                    self.url(args=request.args), client_side=self.settings.client_side
                )
            elif key in ("pending", "disabled", "blocked") or (key or "").startswith(
                "pending"
            ):
                session.flash = self.messages.registration_pending
                redirect(
    ...
    def profile(
        ...
        if form.accepts(
            ...
            extra_fields = self.settings.extra_fields.get(
    ...
    def requires_permission(self, name, table_name="", record_id=0, otherwise=None):
        ...
        def has_permission(
            self=self, name=name, table_name=table_name, record_id=record_id
        ):
    ...
    def accessible_query(self, name, table, user_id=None):
        ...
        if isinstance(table, str) and table in self.db.tables():
            ...
            return cquery
        if not isinstance(table, str) and self.has_permission(name, table, 0, user_id):
            return table.id > 0
        ...
        query = table.id.belongs(
            ...
            )(permission.name == name)(permission.table_name == table)._select(
    ...
    def wikimenu(self):
        ...
        if (
            hasattr(self, "_wiki")
            and self._wiki.settings.controller
            and self._wiki.settings.function
        ):
            self._wiki.automenu()
...
class Crud(object):  # pragma: no cover
    ...
    def tables(self):
        return TABLE(
            *[
                TR(A(name, _href=self.url(args=("select", name))))
    ...
    def archive(form, archive_table=None, current_record="current_record"):
        return Auth.archive(
            form, archive_table=archive_table, current_record=current_record
        )
    ...
    def update(
        ...
        **attributes
        ...
        if next is DEFAULT:
            next = (
                prevent_open_redirect(request.get_vars._next)
        ...
        if request.extension not in ("html", "load"):
            ...
            (_session, _formname) = (
                session,
                "%s/%s" % (table._tablename, form.record_id),
            )
    ...
    def search(self, *tables, **args):
        ...
        for field in fields:
            ...
            row = TR(
                ...
                TD(
                    SELECT(
                        [OPTION(query_labels.get(op, op), _value=op) for op in ops],
                        _name="op" + fieldname,
                        value=opval,
                    )
                ),
            ...
            if request.post_vars and (chkval or field.type == "id"):
                if txtval and opval != "":
                    if field.type[0:10] == "reference ":
                        refsearch.append(
                            self.get_query(field, opval, txtval, refsearch=True)
                        )
                    elif validate:
...
def fetch(
    url, data=None, headers=None, cookie=Cookie.SimpleCookie(), user_agent="Mozilla/5.0"
):
    headers = headers or {}
    if data is not None:
        data = urlencode(data)
...
def reverse_geocode(lat, lng, lang=None):
    ...
    try:
        return json.loads(
            fetch(
                "http://maps.googleapis.com/maps/api/geocode/json?latlng=%(lat)s,%(lng)s&language=%(lang)s"
                % locals()
            )
...
class Service(object):
    ...
    def soap(
        self, name=None, returns=None, args=None, doc=None, response_element_name=None
    ):
        """
        Example:
            Use as::
    ...
    def serve_run(self, args=None):
        ...
        if args and args[0] in self.run_procedures:
            return str(
                self.call_service_function(
                    self.run_procedures[args[0]], *args[1:], **dict(request.vars)
                )
            )
        self.error()
    ...
    def serve_csv(self, args=None):
        ...
        if args and args[0] in self.csv_procedures:
            ...
            r = self.call_service_function(
                self.csv_procedures[args[0]], *args[1:], **dict(request.vars)
            )
            s = StringIO()
            if hasattr(r, "export_to_csv_file"):
                ...
                and isinstance(r[0], (dict, Storage))
            ):
                import csv
    ...
    def serve_xml(self, args=None):
        ...
        if args and args[0] in self.xml_procedures:
            s = self.call_service_function(
                self.xml_procedures[args[0]], *args[1:], **dict(request.vars)
            )
    ...
    def serve_rss(self, args=None):
        ...
        if args and args[0] in self.rss_procedures:
            feed = self.call_service_function(
                self.rss_procedures[args[0]], *args[1:], **dict(request.vars)
            )
        else:
            self.error()
    ...
    def serve_json(self, args=None):
        ...
        if args and args[0] in self.json_procedures:
            s = self.call_service_function(
                self.json_procedures[args[0]], *args[1:], **d
            )
            if hasattr(s, "as_list"):
                s = s.as_list()
    ...
    def serve_jsonrpc2(self, data=None, batch_element=False):
        ...
        def validate(data):
            ...
            if missing_args:
                raise Service.JsonRpcException(
                    -32600, "Missing arguments %s." % list(missing_args)
                )
            if data["jsonrpc"] != "2.0":
                raise Service.JsonRpcException(
                    -32603, 'Unsupported jsonrpc version "%s"' % data["jsonrpc"]
                )
            if "id" not in iparms:
                return False
    ...
    def serve_soap(self, version="1.1"):
        ...
        if request.env.request_method == "POST":
            ...
            body = [
                H1("Welcome to Web2Py SOAP webservice gateway"),
                A(
                ...
                P(doc),
                UL(
                    LI("Location: %s" % dispatcher.location),
                    ...
                    [
                        LI(
                            A(
                                "%s: %s" % (method, doc or ""),
                                _href=URL(
                                    r=request, f="call/soap", vars={"op": method}
                                ),
                            )
                        )
...
class Expose(object):
    def __init__(
        ...
        self.base = base = os.path.realpath(
            base or os.path.join(current.request.folder, "static")
        )
        ...
        if extensions:
            self.filenames = [
                f for f in self.filenames if os.path.splitext(f)[-1] in extensions
            ]
        ...
        if self.filenames:
            return SPAN(
                ...
                TABLE(
                    *[
                        TR(
                            TD(A(f, _href=URL(args=self.args + [f]))),
                            TD(
                                IMG(
                                    _src=URL(args=self.args + [f]),
                                    _style="max-width:%spx" % width,
                                )
                                if width and self.isimage(f)
                                else ""
                            ),
                        )
...
class Wiki(object):
    ...
    def get_renderer(self):
        if isinstance(self.settings.render, basestring):
            ...
            raise ValueError("Invalid render type %s" % type(self.settings.render))
        return r
    ...
    def __init__(
                ...
                "wiki_page",
                {
                    "args": [
                        ...
                        Field(
                            ...
                            requires=IS_EMPTY_OR(IS_IN_SET(engines)),
                        ),
                        auth.signature,
                    ],
        ...
        if self.settings.templates is None and not self.settings.manage_permissions:
            self.settings.templates = db.wiki_page.tags.contains(
                "template"
        ...
        if (
            auth.user
            and check_credentials(current.request, gae_login=False)
            and "wiki_editor" not in auth.user_groups.values()
            and self.settings.groups == list(auth.user_groups.values())
        ):
    ...
    def can_read(self, page):
        if "everybody" in page.can_read or not self.settings.manage_permissions:
            ...
            if (
                "wiki_editor" in groups
                or set(groups).intersection(set(page.can_read + page.can_edit))
                or page.created_by == self.auth.user.id
            ):
                return True
        return False
    ...
    def can_edit(self, page=None):
        ...
        return (
            "wiki_editor" in groups
            or (page is None and "wiki_author" in groups)
            or page is not None
            and (
                set(groups).intersection(set(page.can_edit))
                or page.created_by == self.auth.user.id
            )
        )
    ...
    def automenu(self):
        ...
        if (
            not self.wiki_menu_items
            and self.settings.controller
            and self.settings.function
        ):
            self.wiki_menu_items = self.menu(
                self.settings.controller, self.settings.function
            )
    ...
    def read(self, slug, force_render=False):
        ...
        if current.request.extension == "html":
            if not page:
                ...
                return dict(
                    content=A('Create page "%s"' % slug, _href=url, _class="btn")
                )
            else:
    ...
    def edit(self, slug, from_template=0):
        ...
        if not page:
            if not (self.can_manage() or slug.startswith(self.settings.force_prefix)):
                current.session.flash = (
                    'slug must have "%s" prefix' % self.settings.force_prefix
                )
        ...
        form = SQLFORM(
            db.wiki_page, page, deletable=True, formstyle="table2cols", showid=False
        ).process()
        if form.deleted:
        ...
        """ % dict(
            url=URL(args=("_preview", slug)),
            link_media=("true" if page else "false"),
            urlmedia=URL(
                extension="load", args=("_editmedia", slug), vars=dict(embedded=1)
            ),
        )
    ...
    def editmedia(self, slug):
        ...
        self.auth.db.wiki_media.id.represent = (
            ...
            if not row.filename
                ...
                % (
                    id,
                    IS_SLUG.urlify(row.title.split(".")[0]),
        ...
        if current.request.vars.embedded:
            ...
            links = [
                lambda row: A(
                    "copy into source",
                    _href="#",
                    _onclick=script % (fragment(row.id, row)),
                )
            ]
    ...
    def create(self):
        ...
        fields = [
            Field(
                "slug",
                default=current.request.args(1) or self.settings.force_prefix,
            ...
            fields.append(
                Field(
                    ...
                    requires=IS_EMPTY_OR(
                        IS_IN_DB(
                            db(self.settings.templates), db.wiki_page._id, "%(slug)s"
                        )
                    ),
        ...
        if form.process().accepted:
            form.vars.from_template = (
                0 if not form.vars.from_template else form.vars.from_template
            )
            redirect(
                URL(args=("_edit", form.vars.slug, form.vars.from_template or 0))
            )  # added param
    ...
    def pages(self):
        ...
        self.auth.db.wiki_page.title.represent = lambda title, row: A(
            title, _href=URL(args=row.slug)
        )
        ...
        content = SQLFORM.grid(
            ...
            links=[
                lambda row: A(
                    "edit", _href=URL(args=("_edit", row.slug)), _class="btn"
                ),
                lambda row: A(
                    "media", _href=URL(args=("_editmedia", row.slug)), _class="btn"
                ),
            ],
            details=False,
            editable=False,
            ...
            for match in regex.finditer(self.fix_hostname(menu_page.body)):
                ...
                if link.startswith("@"):
                    ...
                    if len(items) > 3:
                        ...
                        link = URL(
                            a=items[0] or None,
                            c=items[1] or controller,
                            f=items[2] or function,
                            args=items[3:],
                        )
                ...
                tree[base] = subtree
                parent.append(
                    (current.T(title), request.args(0) == title_page, link, subtree)
                )
    ...
    def search(
        ...
        if tags is None and query is None:
            form = FORM(
                INPUT(_name="q", requires=IS_NOT_EMPTY(), value=request.vars.q),
        ...
        if tags or query is not None:
            ...
            if request.extension in ("html", "load"):
                ...
                items = [
                    DIV(
                        H3(A(p.wiki_page.title, _href=URL(args=p.wiki_page.slug))),
                        MARKMIN(self.first_paragraph(p.wiki_page)) if preview else "",
                        DIV(
                            _class="w2p_wiki_tags",
                            *[
                                link(t.strip())
        ...
        for item in ids:
            items.append(
                A(
                    ...
                    _href=URL(args="_search", vars=dict(q=item.wiki_tag.name)),
                )
            )
            items.append(" ")
