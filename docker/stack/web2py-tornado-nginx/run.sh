docker rm $(docker stop $(docker ps -a -q --filter ancestor=web2py-tornado --format="{{.ID}}"))
...
docker build -t web2py-tornado -f web2py-tornado .
docker build -t web2py-nginx -f web2py-nginx .

docker run -d --net web2py-net -v applications:/home/web2py/web2py/applications -p 8000:8000 --name web2py-tornado --ip 172.25.0.22 web2py-tornado
# docker exec -it web2py-tornado  bash

sleep 2
ping -c1 -n 172.25.0.22
docker run --net web2py-net busybox ping -c1 -n web2py-tornado
echo "------------------------------------------------------------------"
