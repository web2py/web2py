...
if os_version == "Windows":
    ...
    if BUILD_DEBUG:  # debug only
        subprocess.call(
            "pyinstaller --clean  --icon=extras/icons/web2py.ico \
    ...
    if BUILD_DEBUG:  # debug only
        subprocess.call(
            "pyinstaller --clean --icon=extras/icons/web2py.icns --hidden-import=gluon.packages.dal.pydal  --hidden-import=gluon.packages.yatl.yatl \
                        --hidden-import=site-packages --windowed web2py.py",
            shell=True,
        )
    ...
    bin_folders = [
        (os.path.join("dist", "web2py.app/Contents/MacOS")),
        (os.path.join("dist", "web2py_cmd")),
    ]
...
print("Your binary version of web2py can be found in " + zip_filename + ".zip")
print(
    "You may extract the archive anywhere and then run web2py without worrying about dependency"
)
print(
    "\nEnjoy binary web2py "
    + web2py_version_line
